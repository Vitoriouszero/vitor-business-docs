...
5. **Apenas 1 e 2 referências foram testadas.** O limite superior *do modelo* segue desconhecido, mas o limite superior *do código* não é: a pipeline aceita **no máximo 2** imagens de referência. O `server.js` monta um array literal de dois elementos com `push` condicional do segundo — sem laço e sem spread —, então 3 referências não são representáveis. O fluxo real **encadeia**: a referência 1 gera a imagem 2; a referência 1 + a referência 2 geram a imagem 3.
...
> **Leia o escopo antes de usar.** Nada nesta subseção faz parte das 10 células da Seção 10, nem das 578 células texto→imagem das Seções 1–9. São medições feitas **depois**, contra um `server.js` **já alterado**. Não some estes números com nenhum dos dois conjuntos, e não os leia nas tabelas 10.3, 10.4 ou 10.5 — eles não estão lá.
